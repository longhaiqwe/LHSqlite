...
#define COLUMN_EMIAL_SIZE 255
typedef struct {
  uint32_t id;
  char username[COLUMN_USERNAME_SIZE + 1];
  char email[COLUMN_EMIAL_SIZE + 1];
} Row;
...
const uint32_t INTERNAL_NODE_RIGHT_CHILD_OFFSET =
    INTERNAL_NODE_NUM_KEYS_OFFSET + INTERNAL_NODE_RIGHT_CHILD_SIZE;
...
uint32_t *internal_node_key(void *node, uint32_t key_num) {
  return internal_node_cell(node, key_num) + INTERNAL_NODE_RIGHT_CHILD_SIZE;
}
...
PrepareResult prepare_insert(InputBuffer *input_buffer, Statement *statement) {
  ...
  if (strlen(email) > COLUMN_EMIAL_SIZE) {
    return PREPARE_STRING_TOO_LONG;
  }
...
PrepareResult prepare_statement(InputBuffer *input_buffer,
  ...
  if (strncmp(input_buffer->buffer, "insert", 6) == 0) {
    return prepare_insert(input_buffer, statment);
  }
  if (strcmp(input_buffer->buffer, "select") == 0) {
    statment->type = STATEMENT_SELECT;
    return PREPARE_SUCCESS;
  }
...
void create_new_root(Table *table, uint32_t right_child_page_num) {
  ...
  // New root node points to two child.
...
// If the key is not present, return th position where it should be inserted.
